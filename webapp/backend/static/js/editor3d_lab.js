...
function showResolutionReport(intent, resolved) {
    ...
    // LAB: log NL resolution outcome + warnings to the bottom console
    logConsole('자연어 해석: ' + status + (hasWarnings ? ' (' + warnings.length + ' warnings)' : ''), hasWarnings ? 'warn' : 'ok');
    warnings.forEach(w => logConsole('NL: ' + (w.message || w), 'warn'));
...
async function uploadIFC() {
    ...
    try {
        ...
        // LAB: surface IFC parse result in the bottom console
        try {
            const val = v2data.validation || {};
            if (val.summary_text) logConsole('IFC: ' + val.summary_text, 'info');
            (val.issues || []).forEach(i => logConsole('IFC ' + (i.severity || 'issue') + ': ' + i.message, 'warn'));
            (val.needs_user_input || []).forEach(i => logConsole('IFC 확인 필요: ' + i.message, 'warn'));
        } catch (_) {}
...
function buildScene(result) {
    ...
    // LAB: refresh the Model Explorer tree from the freshly rendered model
    if (typeof buildModelExplorer === 'function') buildModelExplorer();
}
...
function renderRecommendationsPanel(result) {
    ...
    // LAB: log analysis warnings + issues to the bottom console
    (result.warnings || []).forEach(w => logConsole('경고: ' + (w.message || w), 'warn'));
    if (issues.length) logConsole(issues.length + '개 이슈 감지', 'warn');
    issues.forEach(i => logConsole('이슈: ' + (i.message || i.code || ''), (i.severity === 'high' ? 'error' : 'warn')));
...
function setStatus(text, type) {
    ...
    // LAB: mirror status into the bottom console as an activity trail
    logConsole(text, type === 'error' ? 'error' : (type === 'success' ? 'ok' : 'info'));
}
...
function logConsole(msg, level) {
    ...
    if (line === _labLastLog) return;   // skip consecutive duplicates
    _labLastLog = line;
    const d = new Date();
    const ts = d.toLocaleTimeString('ko-KR', { hour12: false });
    const div = document.createElement('div');
    div.className = 'cl-line cl-' + (level || 'info');
...
// ─── LAB shell: drawer / dock / explorer toggles (ETABS-style layout) ──────
function _labResizeSoon() {
    setTimeout(function () { if (typeof onResize === 'function') onResize(); }, 60);
}
function toggleInputDrawer() {
    const b = document.getElementById('app-body');
    if (b) b.classList.toggle('drawer-collapsed');
    _labResizeSoon();
}
function openInputDrawer() {
    const b = document.getElementById('app-body');
    if (b) b.classList.remove('drawer-collapsed');
    _labResizeSoon();
}
function togglePropertyDock() {
    const b = document.getElementById('app-body');
    if (b) b.classList.toggle('dock-collapsed');
    _labResizeSoon();
}
function toggleModelExplorer() {
    const b = document.getElementById('app-body');
    if (b) b.classList.toggle('explorer-collapsed');
    _labResizeSoon();
}
function switchExplorerTab(name) {
    document.querySelectorAll('.mx-tab').forEach(function (t) {
        t.classList.toggle('active', t.dataset.mxtab === name);
    });
    document.querySelectorAll('.mx-pane').forEach(function (p) {
        p.classList.toggle('active', p.id === 'mxpane-' + name);
    });
}

// ─── LAB Model Explorer: data-driven tree + selection dispatch ─────────────
function selectFromExplorer(sel) {
    if (!sel || typeof clearAllSelection !== 'function') return;
    clearAllSelection();
    if (sel.by === 'element') {
        const entry = memberMeshes.find(m => m.elementData && m.elementData.type !== 'node'
            && String(m.elementData.id) === String(sel.value));
        if (entry) {
            highlightMesh(entry.mesh);
            selectedMesh = entry.mesh;
            if (typeof showMemberProperties === 'function') showMemberProperties(entry.elementData);
        }
    } else if (sel.by === 'story') {
        if (typeof selectByStory === 'function') selectByStory(sel.value);
    } else if (sel.by === 'type') {
        memberMeshes.forEach(({ mesh }) => {
            const d = mesh.userData.elementData;
            if (d && d.type === sel.value && !selectedMeshSet.has(mesh)) highlightMesh(mesh);
        });
    } else if (sel.by === 'section') {
        memberMeshes.forEach(({ mesh }) => {
            const d = mesh.userData.elementData;
            if (d && d.section === sel.value && !selectedMeshSet.has(mesh)) highlightMesh(mesh);
        });
    } else if (sel.by === 'case') {
        const cs = document.getElementById('case-selector');
        if (cs) { cs.value = sel.value; if (typeof onCaseSelect === 'function') onCaseSelect(); }
    }
    if (typeof updateSelectionCount === 'function') updateSelectionCount();
}

function buildModelExplorer() {
    const treeEl = document.getElementById('mx-tree');
    if (!treeEl) return;
    const result = currentResult;
    if (!result || !result.viewer) return;   // keep placeholder until first analysis
    const model = window._v2Model;
    const elements = result.viewer.elements || [];
    const checks = result.member_checks || {};

    // distinct stories from the editable model (has .story)
    let stories = [];
    if (model && model.nodes) {
        const s = new Set();
        model.nodes.forEach(n => { if (n.story != null) s.add(n.story); });
        stories = Array.from(s).sort((a, b) => a - b);
    }
    // group by type + count sections
    const byType = {};
    const sectionCount = {};
    elements.forEach(e => {
        (byType[e.type] = byType[e.type] || []).push(e);
        if (e.section) sectionCount[e.section] = (sectionCount[e.section] || 0) + 1;
    });
    // design check groups
    const dc = { OK: [], NG: [], Other: [] };
    elements.forEach(e => {
        const c = checks[e.id] || checks[String(e.id)];
        if (!c) return;
        const st = (c.status || '').toUpperCase();
        (dc[st === 'OK' ? 'OK' : st === 'NG' ? 'NG' : 'Other']).push(e);
    });

    // ── build DOM ──
    treeEl.innerHTML = '';
    const rootLi = document.createElement('li');
    rootLi.className = 'mx-branch mx-root';
    rootLi.innerHTML = '<span class="mx-twist">▾</span>📁 Model';
    const rootUl = document.createElement('ul');
    rootLi.appendChild(rootUl);
    treeEl.appendChild(rootLi);

    function branch(parentUl, label, cls) {
        const li = document.createElement('li');
        li.className = 'mx-branch' + (cls ? ' ' + cls : '');
        const head = document.createElement('div');
        head.className = 'mx-branch-head';
        head.innerHTML = '<span class="mx-twist">▸</span>';
        ...
        head.addEventListener('click', () => {
            ...
            head.querySelector('.mx-twist').textContent = open ? '▸' : '▾';
        });
        li.appendChild(head);
        li.appendChild(ul);
    ...
    function leaf(parentUl, label, sel, cls) {
        const li = document.createElement('li');
        li.className = 'mx-leaf' + (cls ? ' ' + cls : '');
        li.textContent = label;
        if (sel) li.addEventListener('click', () => selectFromExplorer(sel));
        parentUl.appendChild(li);
        return li;
    }

    const typeLabels = { column: 'Columns', beam_x: 'Beams-X', beam_y: 'Beams-Y' };
    const CAP = 300;

    // Stories
    const uStories = branch(rootUl, '🏢 Stories (' + stories.length + ')');
    stories.forEach(s => leaf(uStories, s === 0 ? 'Base (Z=0)' : 'Story ' + s, { by: 'story', value: s }));

    // Structural Objects
    const uObj = branch(rootUl, '▤ Structural Objects (' + elements.length + ')');
    Object.keys(byType).forEach(t => {
        const list = byType[t];
        const uT = branch(uObj, (typeLabels[t] || t) + ' (' + list.length + ')');
        leaf(uT, '▸ select all', { by: 'type', value: t }, 'mx-selall');
        list.slice(0, CAP).forEach(e => leaf(uT, '#' + e.id + ' · ' + (e.section || ''), { by: 'element', value: e.id }));
        if (list.length > CAP) leaf(uT, '… +' + (list.length - CAP) + ' more', null);
    });

    // Sections
    const secKeys = Object.keys(sectionCount);
    const uSec = branch(rootUl, '▥ Sections (' + secKeys.length + ')');
    secKeys.forEach(sec => leaf(uSec, sec + ' (' + sectionCount[sec] + ')', { by: 'section', value: sec }));

    // Loads
    const cases = (result.case_names || []).concat(result.combo_names || []);
    const uLoads = branch(rootUl, '⭳ Loads — Cases / Combos (' + cases.length + ')');
    cases.forEach(cn => leaf(uLoads, cn, { by: 'case', value: cn }));

    // Design Check
    const dcTotal = dc.OK.length + dc.NG.length + dc.Other.length;
    const uDC = branch(rootUl, '✓ Design Check (' + dcTotal + ')');
    if (dc.NG.length) {
        const uNG = branch(uDC, 'NG (' + dc.NG.length + ')', 'mx-dc-ng');
        dc.NG.slice(0, CAP).forEach(e => leaf(uNG, '#' + e.id + ' · ' + (e.section || ''), { by: 'element', value: e.id }, 'mx-dc-ng'));
    }
    if (dc.OK.length) leaf(uDC, 'OK (' + dc.OK.length + ')', null, 'mx-dc-ok');

    // hide the "run analysis first" hint now that data is present
    const hint = document.querySelector('#mxpane-model .mx-hint');
...
function switchViewerTab(tab) {
    ...
    // Three.js canvas resize (fixed: was calling non-existent onWindowResize)
    setTimeout(() => { if (typeof onResize === 'function') onResize(); }, 120);
}
