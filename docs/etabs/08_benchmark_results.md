...
**총 결과**:
- **Case 1+2+3 (2D)**: 37/37 OK — 세 프로그램 모두 0.04% 이내 일치
- **Case 4 (3D)**: 35/35 OK or CHECK — OpenSees↔ETABS는 **35/35 6 유효숫자 완전 일치**,
  Midas Gen과는 12개 메트릭에서 1.5-3.9% 차이 (3D 골조 모델링 가정 차이로 추정 — 자세한 분석은 §5)

전체 **72개 메트릭에서 FAIL 0건**. OpenSees와 ETABS는 **모든 케이스 모든 메트릭에서 6
유효숫자까지 완전 일치** — 두 독립 솔버가 동일 KS DB 데이터로 동일 답을 내는 강력한
교차검증.
...
| Case | 메트릭 수 | OK | CHECK | FAIL | OS↔ET 일치 | 최대 ET–M |
|------|----------|----|-------|------|------------|-----------|
| 1 | 5 | 5 | 0 | 0 | 6자리 | 0.00 % |
| 2 | 15 | 15 | 0 | 0 | 6자리 | 0.02 % |
| 3 | 17 | 17 | 0 | 0 | 6자리 | 0.04 % |
| 4 | 35 | 23 | 12 | 0 | 6자리 | 3.90 % * |
| **계** | **72** | **60** | **12** | **0** | **6자리** | **3.90 %** |

*Case 4의 12 CHECK는 Midas 측 3D 디폴트(추정: 자동 강체 오프셋)에 의한 모델링
가정 차이로 추정 — §5.3 참조. 솔버 정확성 자체에는 영향 없음.

### 6.2. 핵심 발견

1. **OpenSees ↔ ETABS 모든 72개 메트릭에서 6 유효숫자 일치**: 두 완전 독립 솔버가
...
3. **Midas Gen과 2D는 0.04% 이내 일치**: 단순 평면 골조에서는 세 프로그램 모두 0.04%
   이내 일치 — 출력 자릿수 차이 수준.

4. **Case 복잡도 무관 안정성**: 단순보(5 DOF) → 1층 프레임(24 DOF) → 3층 프레임(48 DOF)
   → 3D 2층 프레임(72 DOF). 시스템 규모가 커져도 OS↔ET 일치도 유지.
...
# ETABS 측 (Case 1·2·3·4)
.\opensees-mcp\Scripts\python.exe tests\benchmark\etabs_benchmark_case1_2.py --launch
# 또는 개별
.\opensees-mcp\Scripts\python.exe tests\benchmark\etabs_benchmark_case1_2.py --launch case4
```
...
| 출처 | 경로 |
|------|------|
| OpenSees | `tests/benchmark/opensees_results/case{1,2,3,4}.json` |
| ETABS | `tests/benchmark/etabs_results/case{1,2,3,4}.json` |
| Midas Gen | `tests/benchmark/midas_results/case{1,2,3,4}.json` |
